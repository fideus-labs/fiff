...
import { computeChunkShape, parseStoreKey } from "./utils.js"
...
export class TiffStore {
  ...
  private indexer: (sel: PlaneSelection, level: number) => Promise<GeoTIFFImage>
  ...
  static async fromGeoTIFF(
    ...
    // For multi-file OME-TIFFs, filter to only the planes in this file
    let ifdMap: Map<string, number> | undefined
    if (pixels && imageDescription) {
      const rootUuid = parseRootUuid(imageDescription)
      const filtered = filterPixelsForFile(pixels, rootUuid)
      if (filtered) {
        pixels = filtered.pixels
        ifdMap = filtered.ifdMap
      }
    }
    ...
    for (let level = 0; level < pyramid.levels; level++) {
      ...
      chunkShapes.push(
        computeChunkShape(levelTileW, levelTileH, dimNames.length),
      )
    }
