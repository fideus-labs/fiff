...
describe("buildAxes", () => {
  ...
  it("orders axes as t, c, z, y, x", () => {
    const { dimNames } = buildAxes(makePixels({ sizeT: 2, sizeC: 3, sizeZ: 4 }))
...
describe("buildCoordinateTransformations", () => {
  ...
  it("builds 2x scale for level 1", () => {
    ...
    const transforms = buildCoordinateTransformations(pixels, pyramid, 1, [
      "y",
      "x",
    ])
  ...
  it("preserves physical sizes in scale", () => {
    ...
    const pyramid = makePyramid()
    const transforms = buildCoordinateTransformations(pixels, pyramid, 0, [
      "y",
      "x",
    ])
...
describe("buildArrayJson", () => {
  it("produces valid Zarr v3 array metadata", () => {
    const json = buildArrayJson([3, 512, 512], [1, 256, 256], "uint16", [
      "c",
      "y",
      "x",
    ])
